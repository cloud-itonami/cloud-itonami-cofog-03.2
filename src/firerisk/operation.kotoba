(ns firerisk.operation
  "InspectionOperationActor -- one fire-risk-inspection coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (InspectionAdvisor) is sealed into a single
  node (:advise); its proposal is ALWAYS routed through the Fire-Risk
  ...
  each operation is auditable and checkpointed. A contractor's
  lifecycle is advanced by MANY ops (inspection logging / risk-finding
  decisions / reinspection scheduling), each independent.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human fire-department dispatcher/inspection supervisor.
  The approver resumes with `{:approval {:status :approved}}` (or
  :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [firerisk.advisor :as advisor]
...
(defn build
  "Compiles an InspectionOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `firerisk.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; InspectionAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Fire-Risk Governor -- independent censor.
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        ...
                        :reason (or reason
                                    (cond (:high-stakes? verdict) :requires-dispatcher-approval
